...
For simplicity, let's make a folder on our desktop called "omics_informatics" and put all our files (the markdown file, the metadata file, and the fasta file) there. Now we need to tell R where this is.
- Mac users can find this by right clicking on the file name in finder, holding down the option key, and clicking "copy FILENAME as path name"
- Windows users can find this by selecting (ie clicking once on) the file and then pressing ctrl+shift+c
